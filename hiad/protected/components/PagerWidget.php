...
class PagerWidget extends CBasePager {
    ...
    public function run()
    {
        ...
        //echo $this->header;//程序原来输出
		$pageCount=$this->getPageCount();//总页数
		if($pageCount == 0) {
			echo $currentPage.'/'.$currentPage;//计算有几页
		} else {
			echo $currentPage.'/'.$pageCount;
		}
        echo CHtml::tag('div',$this->htmlOptions,implode("|",$buttons).$redirect);
    ...
    protected function createRedirect($page)
    {
        $html = '
            <span>跳转到第
            <input type="text" class="input_s" style="width:30px; height:20px; line-height:20px;" id="input_page_redirect-'.Yii::app()->controller->id.'-'.Yii::app()->controller->action->id.'" value="'.$page.'"/>
            页  
            </span><span id="but_redirect" class="btn1" style="float: none;display:inline-block" ><button>跳转</button></span>
            <script type="text/javascript">
                $("#but_redirect").click(function(){
                    var page = $("#input_page_redirect-'.Yii::app()->controller->id.'-'.Yii::app()->controller->action->id.'").val();
                    if(isNaN(page)){
                        $.messager.alert("提示", "请输入一个数字!");
                        return false;
                    }   
                    if(page<=0) {
                    ...
                    var link = "'.$this->createPageUrl(0).'";//window.location.href;
                    var url = "";
                    if (link.indexOf("page=")>0) {
                        url = link.replace(/page=\d+/, "page="+page);
                    } else {
                        url = link+(link.indexOf("?")>0? "&page="+page : "?page="+page);
                    }
                    '.($this->refreshArea != '' ? 'ajax_load("'.$this->refreshArea.'", url);' :'window.location.href=url;').'
                });
            </script>
        ';
        return $html;
        ...
        $script = '
            <script type="text/javascript">
                $("#'.$this->htmlOptions['id'].'").find("a").click(function(){
                    '.($this->refreshArea != '' ? 'ajax_load("'.$this->refreshArea.'", $(this).attr("href"));' :  'window.location.href="href";').'
                    return false;
                });
            </script>
        ';
