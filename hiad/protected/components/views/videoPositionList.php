...
<div class="san_nav">
    ...
    <a href="<?php echo Yii::app()->createUrl('videoPosition/index', array('a' => 'table'));?>" style="line-height: 30px;" class="fr mr_40 load_frame">查看广告位投放视图&gt;&gt;</a>
</div>
<!--end 导航-->
...
<!--生成代码-->
<div class="bgline pl_30 add"><div style="float:left; margin-top:10px;">广告位：</div><a href="<?php echo Yii::app()->createUrl('videoPosition/add');?>" id="add_position" title="新建广告位" class="iscbut cbut_jia load_frame"><span>新建广告位</span></a></div>
...
<div class="tpboder adbox">
    <table border="0" cellspacing="0" cellpadding="0" class="list_table" id="list_table">
        ...
        <th scope="col" width="15%" class="tpboder">广告位ID</th>
        ...
        <tr>
        <td><input type="checkbox" name="ids[]" class="checkbox_ids" value="<?php echo $one->id;?>" /></td>
        <td><?php echo $one->id;?></td>
        <td><?php echo $one->name;?></td>
        <td><?php echo $one->status == 1 ? '启用' : '禁用';?></td>
        <td><?php echo $adShows[$one->ad_show_id];?></td>
        <td><?php echo $one->position_size;?></td>
        <td>
            <a href="<?php echo Yii::app()->createUrl('videoPosition/edit', array('id' => $one->id));?>" class="load_frame" title="修改广告位 <?php echo $one->name;?>">修改</a> |
            <a href="<?php echo Yii::app()->createUrl('videoAd/list', array('positionid' => $one->id, 'position_name' => urlencode($one->name)));?>" title="查看广告" class="load_frame">广告</a>  |
            <a href="<?php echo Yii::app()->createUrl('statistics/video', array('type' => 'position', 'ad_id' => $one->id, 'ad_name' => urlencode($one->name)));?>" class="load_frame">统计</a>
        </td>
        </tr>
        <?php endforeach;?>
          <?php else:?>
