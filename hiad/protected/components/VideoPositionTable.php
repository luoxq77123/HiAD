...
class VideoPositionTable extends CWidget {

    public function run() {
        $user = Yii::app()->session['user'];
        $criteria = new CDbCriteria();
        $criteria->order = 'createtime desc';
        $criteria->select = 'id,name,position_size';
        ...
        $st_with = "(start_time < $month_first_day AND end_time > $month_first_day) OR
                        ...
                        (start_time >= $month_first_day AND end_time <= $month_last_day) OR
                        (start_time < $month_last_day AND end_time=0)";

        // 分页
        $count = Position::model()->count($criteria);
        $pageSize = (isset($_GET['pagesize']) && $_GET['pagesize']) ? $_GET['pagesize'] : 10;
        $pager = new CPagination($count);
        $pager->route = 'videoPosition/table';
        ...
        foreach ($positions as $one) {
            ...
            foreach ($one->AdTime as $t) {
                ...
                if ($t->end_time >= $month_last_day || $t->end_time==0) {
                    $end = $month_last_day;
                } else {
                    $end = $t->end_time;
        ...
        $this->render('videoPositionTable', $set);
    }
