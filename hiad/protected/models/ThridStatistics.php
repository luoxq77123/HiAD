...
class ThridStatistics extends CActiveRecord {
    ...
    public function getAllList($type = 'ad', $typeid = array(), $startDate = "", $endDate = "") {
        ...
        $criteria->addColumnCondition(array('ad_type' => 3));
    ...
    public function getOneDayAllList($type, $typeid, $date) {
        $user = Yii::app()->session['user'];
        $cache_name = md5('model_ThridStatistics_getOneDayAllList_'.$user['com_id'].'_'.$type.'_'.$date);
        if (empty($typeid)) {
        ...
        if ($type == 'material') {
            ...
            $list = VideoStatMate::model($table)->findAll($criteria);
        } else {
            $list = ThridStat::model($table)->findAll($criteria);
        }
        // 将数据库选择到主库
    ...
    public function getPageListByType($type = 'ad', $typeid = array(), $startDate = "", $endDate = "", $route=null) {
        ...
        case 'position': // 广告位
            return StatisticsPosition::model()->getPageList($typeid, 4, $startDate, $endDate, $route);
        case 'order': // 订单
    ...
    public function getDayPageListByType($type, $typeid, $date, $route=null) {
        ...
        // paging
        if ($type == 'material') {
            $count = VideoStatMate::model($table)->count($criteria);
        } else {
            $count = ThridStat::model($table)->count($criteria);
        }
        $criteria->select = $typeField.' as id,'.$typeField.',count('.$typeField.') as show_num, sum(is_click) as click_num, count(distinct(ip)) as dedicgotd_ip, com_id';
    ...
    public function getStatisticsByDate($type = 'ad', $typeid = array(), $startDate = "", $endDate = "") {
        ...
        if ($startDate != "" && $startDate == $endDate) {
            ...
            $return['statTotal'] = $this->getTotalDataByMoreDay($type, $typeid, $adType=4, $startDate, $endDate);
            $return['statChart'] = $this->getChartDataByDay($type, $typeid, $adType=4, $startDate, $endDate);
        }
        return $return;
    }
    ...
    public function getTotalDataByOneDay($type = 'ad', $typeid = array(), $date = ""){
        ...
        // 根据条件查询统计信息
        $criteria = new CDbCriteria();
        $criteria->select = "count($typeField) as show_num, sum(is_click) as click_num";
        $criteria->addColumnCondition(array("com_id" => $user['com_id']));
        $criteria->addCondition($typeField."!=0");
        if (is_array($typeid) && !empty($typeid)) {
            $criteria->addInCondition($typeField, $typeid);
        ...
        if ($type == 'material') {
            ...
            $rData = VideoStatMate::model($table)->find($criteria);
        } else {
            $rData = ThridStat::model($table)->find($criteria);
        }
        $arrData = array();
        if (!empty($rData)) {
    ...
    public function getChartDataByHour($type = 'ad', $typeid = array(), $date = ""){
        $user = Yii::app()->session['user'];
        $cache_name = md5('model_ThridStatistics_getChartDataByHour_'.$user['com_id'].'_'.$type.'_'.$date);
        if (empty($typeid)) {
            // only cached data of yesterday or today
            if ($date==date("Y-m-d", time()) || $date==date("Y-m-d", time()-86400)) {
                $data = Yii::app()->memcache->get($cache_name);
        ...
        if ($type == 'material') {
            ...
            $rData = VideoStatMate::model($table)->findAll($criteria);
        } else {
            $rData = ThridStat::model($table)->findAll($criteria);
        }
        $arrData = array();
        if (!empty($rData)) {
    ...
    public function parseParams(){
        $type = isset($_GET['type'])&&$_GET['type']!=""? $_GET['type'] : "position";
