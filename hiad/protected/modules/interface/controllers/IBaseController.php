 ...
 * 基础controller
 */
class IBaseController extends CController {
    ...
    function __construct($id, $module = null) {
        ...
        if (isset($_POST['appId'])) {
            $app = ComApp::model()->getById($_POST['appId']);

            $this->app = $app;
            $params = $app ? @json_decode(HmPassport::decrycode($_POST['parameter'], $app->appkey), true) : '';

            // 去除用户输入首尾空格
            $this->params = HmString::clean_space($params, TRUE);
            $this->params['com_id'] = isset($app->com_id) ? $app->com_id : '';

            $this->Registration($params);
        }
        if(isset($_GET['appId'])){ 
            $this->key ='4F101363';
            $this->params = @json_decode($this->bspDecrypt($_GET['code'],$this->key),true);
            $this->params['com_id'] = 1;
            //$this->Registration($this->params);
        }
    }
