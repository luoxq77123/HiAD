                ...
                <tr>
                    ...
                    <td>
                        <div class="option">
                            ...
                            <a href="<?php echo Yii::app()->createUrl('materialVideo/list', array('aid' => $one->id, 'ad_name' => urlencode($one->name)));?>" title="查看广告物料" class="load_frame">广告物料</a> | 
...
<?php endif; ?>
    ...
    $(function(){
        ...
        $("#z_start").click(function(){
            ...
            if (aids=="") 
                return false;
            $.post(
            '<?php echo Yii::app()->createUrl("videoAd/setStartAd") ?>',
            {'aids':aids}, 
            function(data){
                if(data.code < 1){
                    jAlert(data.msg);
                    ...
                    var url = (arrUrl.length>1)? '<?php echo Yii::app()->createUrl("videoAd/list") ?>?'+arrUrl[1] : '<?php echo Yii::app()->createUrl("videoAd/list") ?>';
        ...
        $("#z_stop").click(function(){
            ...
            if (aids=="") 
                return false;
            $.post(
                '<?php echo Yii::app()->createUrl("videoAd/setStopAd") ?>',
                {'aids':aids}, 
                function(data){
                   //  alert(data);
                    if(data.code < 1){
        ...
        $("#z_dele").click(function(){
            ...
            $.post(
                '<?php echo Yii::app()->createUrl("videoAd/del") ?>',
                {'aids':aids}, 
                function(data){
                    if(data.code < 1){
                        jAlert(data.message);
        ...
        $("#add_new_ad").click(function(){
            ...
            var url = "<?php echo Yii::app()->createURL('videoAd/setAd?do=create'); ?>";
            if (positionId>0) {
    ...
    function submit_search(){
        ...
        var url = '<?php echo Yii::app()->createUrl('videoAd/list', $get) ?>&status='+search_status+'&name='+encodeURIComponent(search_name);
    ...
    function modifyAd(aid){
        var url = "<?php echo Yii::app()->createURL('videoAd/setAd?do=modify&aid='); ?>"+aid;
        frame_load(url);
    }
    
    function viewStatistics(aid, adName) {
        var url = "<?php echo Yii::app()->createURL('statistics/video?ad_id='); ?>"+aid+"&ad_name="+encodeURIComponent(adName);
        frame_load(url);
        return false;
    }
</script>