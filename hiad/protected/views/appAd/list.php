...
<div class="tpboder pl_35" id="pt">
    <div class="z"><a href="javascript:void(0);">投放</a> &gt; <a href="javascript:void(0);">客户端广告</a></div>
</div>
<!--end 面包屑-->
<div class="taskbar">
                ...
                <tr>
                    ...
                    <td>
                        <div class="option">
                            ...
                            <a href="<?php echo Yii::app()->createUrl('materialApp/list', array('aid' => $one->id, 'ad_name' => urlencode($one->name)));?>" title="查看广告物料" class="load_frame">广告物料</a> | 
...
<?php endif; ?>
    ...
    $(function(){
        ...
        for(var i=0; i<len; i++){
            ...
            $btnAs=$thisTr.find(".option").children("a").last();
            $btnAs.click( function(){
                $(this).parent(".option").parent("td").parent("tr").remove();
            })
        ...
        $("#z_start").click(function(){
            ...
            if (aids=="") 
                return false;
            $.post(
            '<?php echo Yii::app()->createUrl("appAd/setStartAd") ?>',
            {'aids':aids}, 
            function(data){
                if(data.code < 1){
                    jAlert(data.msg);
                    ...
                    var url = (arrUrl.length>1)? '<?php echo Yii::app()->createUrl("appAd/list") ?>?'+arrUrl[1] : '<?php echo Yii::app()->createUrl("appAd/list") ?>';
        ...
        $("#z_stop").click(function(){
            ...
            if (aids=="") 
                return false;
            $.post(
                '<?php echo Yii::app()->createUrl("appAd/setStopAd") ?>',
                {'aids':aids}, 
                function(data){
                    if(data.code < 1){
                        jAlert(data.msg);
        ...
        $("#z_dele").click(function(){
            ...
            $.post(
                '<?php echo Yii::app()->createUrl("appAd/del") ?>',
                {'aids':aids}, 
                function(data){
                    if(data.code < 1){
                        jAlert(data.message);
        ...
        $("#add_new_ad").click(function(){
            ...
            var url = "<?php echo Yii::app()->createURL('appAd/setAd?do=create'); ?>";
            if (positionId>0) {
    ...
    function submit_search(){
        ...
        var url = '<?php echo Yii::app()->createUrl('appAd/list', $get) ?>&status='+search_status+'&name='+encodeURIComponent(search_name);
    ...
    function modifyAd(aid){
        var url = "<?php echo Yii::app()->createURL('appAd/setAd?do=modify&aid='); ?>"+aid;
        frame_load(url);
    }
    
    function viewStatistics(aid, adName) {
        var url = "<?php echo Yii::app()->createURL('statistics/app?ad_id='); ?>"+aid+"&ad_name="+encodeURIComponent(adName);
        frame_load(url);
    }
</script>