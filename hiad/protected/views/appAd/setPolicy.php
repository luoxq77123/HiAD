                    ...
                    <div id="cont_w7check" class="w7Ttit2 <?php if(!$policy['set_cost'])echo 'hide'; ?>">
                        ...
                        <ul>
                            ...
                            <li class="help">
                                ...
                                <div id="error_cost_set" class="hide errmsg">提示：数量请填写>0且≤1,000,000,000的整数。</div>
                            </li>
                            <div style="clear:both"></div>
                        </ul>
                        <div class="feeAll">
                            <strong class="mgl5">总费用</strong>
    ...
    <table border="0" cellpadding="0" cellspacing="0">
        <tbody>
            <tr>
                ...
                <td><a href="javascript:void(0);" onclick="addDirectional();"><img src="<?php echo Yii::app()->request->baseUrl; ?>/images/btn10.gif" /></a>&nbsp;&nbsp;<a class="toolTips_tag" onmouseover="showMyToolTips(this, '什么是精准定向？', '精准定向是从各个维度对网站访客的筛选，实现根据访客的不同特征来展现不同的广告。')" onmouseout="hideMyToolTips()" href="javascript:void(0);"><img src="<?php echo Yii::app()->request->baseUrl; ?>/images/tit4.gif" /></a></td></td>
            </tr>
            <tr>
                <td width="84">&nbsp;</td>
                <td width="1000" id="directional_value">
                  <ul class="directional_list" id="directional_list">
                    <?php if(isset($directional['area_set'])) { ?>
                    <li id="area_set" class="help">
                      <img onclick="delAreaSet();" src="<?php echo Yii::app()->request->baseUrl; ?>/images/deltit.gif">
                      地域 = <?php echo $directional['area_set']['text'];?>
                      <input id="area_set_text" type="hidden" value="<?php echo $directional['area_set']['value'];?>" name="area_set_text">
                      [
                      <a onclick="showDirectional(1)" href="javascript:void(0);">修改</a>
                      ]
                    </li>
                    <?php } ?>
                    <?php if(isset($directional['connect_set'])) { ?>
                    <li id="connect_set" class="help">
                      <img onclick="delConnectSet();" src="<?php echo Yii::app()->request->baseUrl; ?>/images/deltit.gif">
                      接入方式 = <?php echo $directional['connect_set']['text'];?>
                      <input id="connect_set_text" type="hidden" value="<?php echo $directional['connect_set']['value'];?>" name="connect_set_text">
                      [
                      <a onclick="showDirectional(2)" href="javascript:void(0);">修改</a>
                      ]
                    </li>
                    <?php } ?>
                    ...
                    <?php } ?>
                    <?php if(isset($directional['brand_set'])) { ?>
                    <li id="brand_set" class="help">
                      <img onclick="delContentSet('brand');" src="<?php echo Yii::app()->request->baseUrl; ?>/images/deltit.gif">
                      手机品牌 = <?php echo $directional['brand_set']['text'];?>
                      <input id="brand_set_text" type="hidden" value="<?php echo $directional['brand_set']['value'];?>" name="brand_set_text">
                      [
                      <a onclick="showDirectional(4)" href="javascript:void(0);">修改</a>
                      ]
                    </li>
                    <?php } ?>
                    <?php if(isset($directional['platform_set'])) { ?>
                    <li id="platform_set" class="help">
                      <img onclick="delContentSet('platform');" src="<?php echo Yii::app()->request->baseUrl; ?>/images/deltit.gif">
                      投放平台 = <?php echo $directional['platform_set']['text'];?>
                      <input id="platform_set_text" type="hidden" value="<?php echo $directional['platform_set']['value'];?>" name="platform_set_text">
                      [
                      <a onclick="showDirectional(6)" href="javascript:void(0);">修改</a>
                      ]
                    </li>
                    <?php } ?>
                    <?php if(isset($directional['resolution_set'])) { ?>
                    <li id="resolution_set" class="help">
                      <img onclick="delContentSet('resolution');" src="<?php echo Yii::app()->request->baseUrl; ?>/images/deltit.gif">
                      分辨率 = <?php echo $directional['resolution_set']['text'];?>
                      <input id="resolution_set_text" type="hidden" value="<?php echo $directional['resolution_set']['value'];?>" name="resolution_set_text">
                      [
                      <a onclick="showDirectional(7)" href="javascript:void(0);">修改</a>
                      ]
                    </li>
                    <?php } ?>
                  </ul>
            ...
            <tr>
                <td width="84">&nbsp;</td>
                <td width="1000" id="directional" class="hide">
                  <?php $this->widget('AppAdDirectionalWidget', array()); ?>
                </td>
            </tr>
        </tbody>
    </table>
...
<div class="nextBtn">
    <a href="javascript:void(0);" id="step_next"><img src="<?php echo Yii::app()->request->baseUrl; ?>/images/btn7.gif"></a><a href="javascript:void(0);" id="step_prev"><img src="<?php echo Yii::app()->request->baseUrl; ?>/images/btn8.gif"></a>
</div>
...
<script type="text/javascript">
    $(function(){
        ...
        $("#step_prev").click(function(){
            var url = "<?php echo Yii::app()->createURL('appAd/setAd');?>";
            frame_load(url);
            return false;
        })
        // 下一步点击事件
    ...
    function submitForm(){
        ...
        var brandSet = ($("#brand_set_text").length>0)? $("#brand_set_text").val() : 0;
        //var blanguageSet = ($("#blanguage_set_text").length>0)? $("#blanguage_set_text").val() : 0;
        var platformSet = ($("#platform_set_text").length>0)? $("#platform_set_text").val() : 0;
        ...
        if (set_cost) {
            if (price!="" && !re.test(price)) {
                ...
                if (!re.test(cost_num) || cost_num<1 || cost_num>1000000000) {
                    setError("error_cost_set", true);
                    check = false;
                } else {
        ...
        $.post(
            '<?php echo Yii::app()->createUrl("appAd/setPolicy")?>',
            {'do':'save','aid':aid,'time_mode':time_mode,'start_time':start_time,'set_endtime':set_endtime,'end_time':end_time,'gap_time':gap_time,'priority_mode':priority_mode,'priority':priority,'set_weights':set_weights,'weights':weights,'set_cost':set_cost,'cost_mode':cost_mode,'price':price,'set_cost_num':set_cost_num,'cost_num':cost_num,'set_limit_day':set_limit_day,'limit_day_mode':limit_day_mode,'limit_day_num':limit_day_num,'set_limit_one':set_limit_one,'limit_one':limit_one,'areaSet':areaSet,'connectSet':connectSet,'timeSet':timeSet,'brandSet':brandSet,'platformSet':platformSet,'resolutionSet':resolutionSet/*,'fromurlSet':fromurlSet,'accessurlSet':accessurlSet*/}, 
            function(data){
                if(data.code < 1){
                    jAlert(data.message);
                }else{
                    var url = "<?php echo Yii::app()->createURL('appAd/setMaterial?aid=');?>"+aid;
                    frame_load(url);
                }
            },
            'json'
    ...
    function saveConnectSet(){
        ...
        if ($("#connect_set").length>0) {
            ...
            $("#connect_select_box li").each(function(){
                ...
                html += $("#"+tmpid+" span").html()+",";
                selectNum ++;
                var tmpConnectId = tmpid.replace("connect_select_", "");
                pText += (pText=="")? tmpConnectId : ","+tmpConnectId;
            });
            if (selectNum == 0) {
                return false;
            }
