...
<div class="tpboder pl_35" id="pt">
    <div class="z"><a href="#">物料库</a> &gt; <a href="#">视频物料库</a></div>
</div>
<!--end 面包屑-->
<!--按钮-->
<div class="tpboder pl_20 adbox">
    <div class="lxr_sx"><a id="material_add" href="<?php echo $this->createUrl('materialVideo/add'); ?>" title="新建物料" class="iscbut cbut_jia"><span>新建物料</span></a>		</div>
</div>
<!--end 按钮-->
<!--提示-->
<div class="taskbar">
       ...
       <?php if($materiallist):?>
        <?php foreach ($materiallist as $one): ?>
            <tr>
                ...
                <td>
                    <a href="<?php echo $this->createUrl('materialVideo/edit', array('id' => $one->id)); ?>" title="修改物料信息" class="load_frame">修改</a> |
                    <!--<a href="#">广告</a> | 
                    <a href="#">报告</a>-->
                    <a target="_blank" href="<?php echo Yii::app()->createUrl('client/cbad', array('val' => $one->id, 'ad_type' => 3, 'type'=>$one->material_type_id));?>" onclick="">预览</a>
                </td>
            </tr>
        <?php endforeach; ?>
          <?php else:?>
...
<script type="text/javascript">
    ...
    function material_search(){
        ...
        var url = '<?php echo Yii::app()->createUrl('materialVideo/list', $get) ?>&status='+status+'&type='+type+'&size='+encodeURIComponent(size)+'&name='+encodeURIComponent(name);
        //alert(url);
    ...
    function material_status(status, uid){
        ...
        $.post(
        '<?php echo $this->createUrl('materialVideo/status'); ?>',
        ...
        function(data){
            if(data.code < 0){
                ...
                setTimeout('frame_load("<?php echo $this->createUrl('materialVideo/list'); ?>", true)', 1000);
    ...
    function material_delete(id){
        ...
        jConfirm('是否删除用户？', '提示', function(r){
            if(r){
                banner_message('后台处理中，请稍后');
                $.post(
                '<?php echo $this->createUrl('materialVideo/del'); ?>', 
                {'material[]':material}, 
                function(data){
                    if(data.code < 0){
                        banner_message(data.message);
