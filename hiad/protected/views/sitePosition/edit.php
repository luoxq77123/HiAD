...
$form = $this->beginWidget('CActiveForm', array(
            ...
            'action' => array('sitePosition/edit?id='.$_GET['id']),
            'clientOptions' => array(
...
<table class="webadd new_web"  border="0" cellpadding="10" cellspacing="10">
    ...
    <tr>
        ...
        <ul>
          <li><a href="javascript:void(0);"><img src="<?php echo Yii::app()->request->baseUrl; ?>/images/tit4.gif" />
            <div>
              <dt>广告位类型有哪些？</dt>
              <dd style="text-indent:0em !important;text-indent: -3.4em;">固定：在页面上占据固定位置的广告位。<br />
    ...
    <tr class="<?php if($position['position_size']!="" && !in_array($position['position_size'],$sizes)) echo "hiden"; ?> size_select" id="size_default">
    ...
    <tr class="<?php if($position['position_size']=="" || in_array($position['position_size'],$sizes)) echo "hiden"; ?> size_select" id="size_defined">
        <th><span class="notion">*</span><?php echo $form->label($position, 'position_size');?></th>
        <td>
            <input type="hidden" name="size_defined" value="0" />
            <input type="text" value="<?php $tmp=explode("*",$position['position_size']); echo $position['position_size']==""? "宽" : reset($tmp); ?>" name="size_x" class="txt1 size_input" />&nbsp;*&nbsp;<input type="text" name="size_y" value="<?php echo $position['position_size']==""? "宽" : next($tmp); ?>" class="txt1 size_input" />&nbsp;(px)
    ...
    <tr>
        ...
        <td><div class="system_tips_box_td">
            <?php echo $form->dropDownList($sitePosition, 'site_id', $sites, array('class' => 'txt1')); ?>
            <a href="<?php echo $this->createUrl('site/add'); ?>" class="suosu" id="add_gs" title="新建站点">新建站点</a> 
    ...
    <tr valign="top" class="ad_type_more pop_more <?php if(!($position->ad_show_id == 3)) echo 'hiden'?>">
        <th><?php echo $form->label($sitePosition, 'poptime');?></th>
        <td>
            <?php echo $form->radioButtonList($sitePosition, 'poptime', SitePosition::model()->getPopTimeOption(), array('separator' => ' '));?>
        </td>
    </tr>
    <tr valign="top" class="ad_type_more float_more pop_more <?php if(!($position->ad_show_id == 3 || $position->ad_show_id == 2 )) echo 'hiden'?>">
        <th>屏幕位置</th>
        <td>
            <?php echo $form->radioButtonList($sitePosition, 'float_x', SitePosition::model()->getFloatXOption(), array('separator' => ' '));?>&nbsp;&nbsp;
            <?php echo $form->label($sitePosition, 'space_x');?>：<?php echo $form->textField($sitePosition, 'space_x', array('class' => 'txt1 pssinput')) ;?>&nbsp;(px)
            <label for="SitePosition_space_x" generated="true" class="error"></label>
            <br/>
            <?php echo $form->radioButtonList($sitePosition, 'float_y', SitePosition::model()->getFloatYOption(), array('separator' => ' '));?>&nbsp;&nbsp;
            <?php echo $form->label($sitePosition, 'space_y');?>：<?php echo $form->textField($sitePosition, 'space_y', array('class' => 'txt1 pssinput')) ;?>&nbsp;(px)
            <label for="SitePosition_space_y" generated="true" class="error"></label>
        </td>
    </tr>
    <tr valign="top" class="ad_type_more float_more pop_more <?php if(!($position->ad_show_id == 3 || $position->ad_show_id == 2 )) echo 'hiden'?>">
        <th><?php echo $form->label($sitePosition, 'staytime');?></th>
        <td>
            <span id="st_unlimit" class="<?php if($sitePosition['staytime']>0) echo "hiden"; ?> st_limit">不限&nbsp;<a href="javascript:void(0);" style="margin:0;" onclick="st_limit('limit');">更改</a></span>
            <span id="st_limit" class="<?php if($sitePosition['staytime']==0) echo "hiden"; ?> st_limit">
            ...
            <span class="ad_type_more float_more"><?php echo $form->checkBox($sitePosition, 'scroll');?>&nbsp;<?php echo $form->label($sitePosition, 'scroll');?></span>
            <label for="SitePosition_staytime" generated="true" class="error"></label>
        </td>
    </tr>
    <tr valign="top">
...
<div class="add hicreat ad_type_more fixed_more <?php if(!($position->ad_show_id == 1 )) echo 'hiden'?>"">
    ...
    <div id="duo" style="display:none;"><a href="javascript:void(0);" onclick="javascript:xZhankai('duo','shao');"><span>︽</span>高级设置</a>
        <p> 
            <?php echo $form->label($sitePosition, 'idle_take');?>
            <?php echo $form->radioButtonList($sitePosition, 'idle_take', SitePosition::model()->getIdleTakeOption(), array('separator' => ' '));?>
