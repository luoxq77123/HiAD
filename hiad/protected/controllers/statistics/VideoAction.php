...
class VideoAction extends CAction {
    ...
    public function initParams() {
        ...
        // 统计时间选择参数
        $params['timePeriods'] = VideoStatistics::model()->timePeriodsParams();
    ...
    public function getStatisticsInfo() {
        $params = VideoStatistics::model()->parseParams();
        $return = array();
        ...
        // 根据条件获取统计数据
        $statisticsData = VideoStatistics::model()->getStatisticsByDate($params['type'], $params['typeid'], $params['startDate'], $params['endDate']);
    ...
    public function combineData($statisticsData = array(), $type = "ad_id", $startDate = "", $endDate = "") {
        ...
        if ($startDate == $endDate) {
            ...
            for ($i = 0; $i < 24; $i++) {
                $timing = VideoStatistics::model()->mstrToTime($startDate . " " . sprintf("%02d", $i+8) . ":00:00");
            ...
            $unixSTime = VideoStatistics::model()->mstrToTime($startDate . " 01:00:00");
            $unixETime = VideoStatistics::model()->mstrToTime($endDate . " 01:00:00");
            $index = 0;
            $statChart = $statisticsData['statChart'];
            for ($i = $unixSTime; $i <= $unixETime; $i+=86400) {
                $date = date("Y-m-d", $i);
                // 08:00:00 是由于highcharts绘图格林威治时间 需+8
                $timing = VideoStatistics::model()->mstrToTime($date . " 08:00:00");
