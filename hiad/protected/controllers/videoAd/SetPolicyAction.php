...
class SetPolicyAction extends CAction {
    public function run(){
        ...
        // 优先级方式
        $priorityMode = VideoAd::model()->getPriorityMode();
        // 优先级参数列表
        $priorityList = VideoAd::model()->getPriorityList();
        // 权重参数列表
        $weightList = VideoAd::model()->getWeightList();
        // 计费方式
        $costMode = VideoAd::model()->getCostMode();
        // 显示每日投放数量方式
        $limitDayMode = VideoAd::model()->getLimitDayMode();
        // 限制对独立访客的展现次数方式
        $limitOneShowMode = VideoAd::model()->getLimitOneShowMode();
    ...
    private function _saveTempPolicy(){
        ...
        $tposX = 0;
        $tposY = 0;
        if ($adInfo['ad']['ad_show'] == "播放器" || $adInfo['ad']['ad_show'] == "视频栏目" || $adInfo['ad']['ad_show'] == "直播频道") {
            $tcushion = $cushion;
            $twidth = $width;
            $theight = $height;
            $tposX = $pos_x;
