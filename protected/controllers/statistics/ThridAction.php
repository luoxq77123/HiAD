 ...
 * 站点广告统计动作。
 */
class ThridAction extends CAction {
    public function run() {
        $set = $this->initParams();
        // 统计
        ...
        $controller->renderPartial('thrid', $set);
    }
    ...
    public function initParams() {
        ...
        // 统计时间选择参数
        $params['timePeriods'] = ThridStatistics::model()->timePeriodsParams();
    ...
    public function getStatisticsInfo() {
        ...
        // 根据条件获取统计数据
        $statisticsData = ThridStatistics::model()->getStatisticsByDate($params['type'], $params['typeid'], $params['startDate'], $params['endDate']);
    ...
    public function combineData($statisticsData = array(), $type = "ad_id", $startDate = "", $endDate = "") {
        ...
        if ($startDate == $endDate) {
            ...
            for ($i = 0; $i < 24; $i++) {
                $timing = ThridStatistics::model()->mstrToTime($startDate . " " . sprintf("%02d", $i+8) . ":00:00");
            ...
            $unixSTime = ThridStatistics::model()->mstrToTime($startDate . " 01:00:00");
            $unixETime = ThridStatistics::model()->mstrToTime($endDate . " 01:00:00");
            $index = 0;
            $statChart = $statisticsData['statChart'];
            for ($i = $unixSTime; $i <= $unixETime; $i+=86400) {
                $date = date("Y-m-d", $i);
                // 08:00:00 是由于highcharts绘图格林威治时间 需+8
                $timing = ThridStatistics::model()->mstrToTime($date . " 08:00:00");
