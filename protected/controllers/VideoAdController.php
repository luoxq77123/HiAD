 ...
 * 广告控制器
 */
class VideoAdController extends BaseController {
    ...
    public function actions() {
        return array(
            'setAd' => 'application.controllers.videoAd.SetAdAction', //设置广告
            'setPolicy' => 'application.controllers.videoAd.SetPolicyAction', //设置投放策略
            'setMaterial' => 'application.controllers.videoAd.SetMaterialAction', //设置素材
        );
    }
    ...
    public function actionGetMaterialList() {
        ...
        $this->renderPartial('materialList', $set);
    }
    ...
    public function actionGetMaterialInfo() {
        ...
        foreach ($mlist as $val) {
            ...
            $material .= '<span class="sw_150"> <a href="' . $this->createUrl('material/edit', array('id' => $val->id)) . '" title="修改物料信息" class="load_frame">修改</a> | <a target="_blank" href="' . Yii::app()->createUrl('client/cbad', array('val' => $val->id, 'ad_type'=>2, 'type' => $val->material_type_id)) . '" onclick="">预览</a> | <a href="javascript:void(0);" onclick="removeCutData(\'' . $val->id . '\')">移除</a></span>';
            ...
            if ($val->material_type_id == 1) {
                ...
                $content = '<a href="' . $data->click_link . '" target="_blank"><img src="' . Yii::app()->request->baseUrl.$data->url . '" alt="物料图片" onerror="this.src=\''.Yii::app()->request->baseUrl.'/images/none.png\';"  width="'.$data->pic_x.'" height="'.$data->pic_y.'" /></a>';
