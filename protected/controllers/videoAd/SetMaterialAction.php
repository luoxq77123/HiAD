...
class SetMaterialAction extends CAction {
    ...
    public function getMaterialHtml($aid){
        ...
        if ($adInfo['ad']['aid']==$aid && !empty($adInfo['material'])){
            ...
            if ($mrotate_mode=="" || $material==""){
                $adData = VideoAd::model()->getOneByAdId($aid);
                if (empty($adData)) {
                    return false;
                }
            ...
            foreach($mlist as $val){
                ...
                $materialText .= '<span class="sw_150" style="display:none;">权重'.CHtml::dropDownList('mweights_'.$val->id, @$arrWeigths[$val->id], VideoAd::model()->getWeightList(), array('class' => 'select_box ml15', 'id' => 'mweights_'.$val->id)).'</span>';
                $materialText .= '<br />';
                ...
                if ($val->material_type_id==1) {
                    ...
                    $content = '<a href="'.$data->click_link.'" target="_blank"><img src="'.Yii::app()->request->baseUrl.$data->url.'" onerror="this.src=\''.Yii::app()->request->baseUrl.'/images/none.png\';" /></a>';
    ...
    private function _saveMaterialSet(){
        ...
        $VideoAd->updateAll($attribe, 'ad_id=:ad_id and com_id=:com_id', array(':ad_id'=>$aid,':com_id'=>$user['com_id']));
