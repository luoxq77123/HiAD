...
class AppAdController extends BaseController {
    ...
    public function actions() {
        return array(
            'setAd' => 'application.controllers.appAd.SetAdAction', //设置广告
            'setPolicy' => 'application.controllers.appAd.SetPolicyAction', //设置投放策略
            'setMaterial' => 'application.controllers.appAd.SetMaterialAction', //设置素材
        );
    }
    ...
    public function actionList() {
        ...
        // 根据条件获取广告列表和分页信息
        $adData = Ad::model()->getPagerList(array(), 2);
        $adlist = $adData['list'];
        $pager = $adData['pager'];
    ...
    public function actionGetAdPositionList() {
        $this->renderPartial('appAdPositionList');
    }
    ...
    public function actionGetMaterialInfo() {
        ...
        foreach ($mlist as $val) {
            ...
            $material .= '<span class="sw_150"> <a href="' . $this->createUrl('materialApp/edit', array('id' => $val->id)) . '" title="修改物料信息" class="load_frame">修改</a> | <a target="_blank" href="' . Yii::app()->createUrl('client/cbad', array('val' => $val->id, 'ad_type'=>2, 'type' => $val->material_type_id)) . '" onclick="">预览</a> | <a href="javascript:void(0);" onclick="removeCutData(\'' . $val->id . '\')">移除</a></span>';
