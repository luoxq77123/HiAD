...
<div class="taskbar">
  <div class="line line6">
    <ul>
      <li>第一步：广告设置</li>
      <li class="activeStep">第二步：投放策略</li>
      <li>第三步：上传素材</li>
    </ul>
  </div>
</div>
<div class="w6Table w7Table mainForm">
    <input type="hidden" name="ad_id" id="ad_id" value="<?php echo $policy['aid']; ?>" />
    <table border="0" cellpadding="0" cellspacing="0">
        <tbody>
            <tr>
                <td width="84" height="34" valign="top"><strong>投放时间：</strong></td>
                <td class="ad_policy_frame">
                    <table border="0" cellpadding="0" cellspacing="0">
                    <tr>
                    <td width="450" valign="top">
                    <input type="hidden" name="time_mode" id="time_mode" value="<?php echo $policy['time_mode']; ?>" /> 
                    <div id="policy_default_time" <?php if($policy['time_mode']=="")echo 'style="display:none;"'; ?>>
                    <ul>
                        <li class="w7Ttit"><span class="notion">*</span><strong>开始</strong></li>
                        <li>
                    ...
                    <ul>
                        <li class="w7Ttit"><span class="notion">*</span><strong>结束</strong></li>
                    ...
                    <td svalign="top">
                    <div>
                         <strong class="mgl5">优先级</strong>
                         <?php echo CHtml::dropDownList('priority_mode', @$policy['priority_mode'], $priorityMode, array('class' => 'select_box ml15', 'id' => 'priority_mode', 'onchange'=>'setPriority()')); ?>
                         <?php echo CHtml::dropDownList('priority', @$policy['priority'], $priorityList, array('class' => 'select_box', 'id' => 'priority')); ?>
                         <a  onmouseover="showMyToolTips(this, '什么是优先级？', '优先级决定广告的展现顺序。<br />优先级：独占>标准>补余>底层。<br />1.独占：支持按日计费。<br />2.标准：默认的优先级，细分为1-10级。<br />&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; 优先顺序依次为10级>9级>...>1级。<br />3.补余：优先级低于独占和标准。<br />4.底层：通常用于占位广告，例如广告位招租等。')" onmouseout="hideMyToolTips()"  class="toolTips_tag " href="javascript:void(0);"><img src="<?php echo Yii::app()->request->baseUrl; ?>/images/tit4.gif" /></a>
                         <input type="checkbox" class="ml15" name="weights" id="weights" <?php if($policy['set_weights']) echo 'checked="true"';?> /><label for="weights">&nbsp;设置权重</label>
                         ...
                         <?php } ?>
                         <a class="toolTips_tag" onmouseover="showMyToolTips(this, '什么是权重？', '权重：优先级相同时，高权重的广告的展现概率更高。')" onmouseout="hideMyToolTips()"  href="javascript:void(0);"><img src="<?php echo Yii::app()->request->baseUrl; ?>/images/tit4.gif" /></a>
                    </div>
                    <div class="mgl5 mt15">
                        <input id="w7check" type="checkbox" <?php if($policy['set_cost']) echo 'checked="true"';?> /><label for="w7check" class="w7Ttit1">设置投放数量和价格</label>
                        <a class="toolTips_tag" onmouseover="showMyToolTips(this, '数量和价格是否必须设置？', '不是必须设置。如果不设置，广告将按计费方式为“每日费用”、价格为“0”、数量为“不限”进行投放。')" onmouseout="hideMyToolTips()"  href="javascript:void(0);"><img src="<?php echo Yii::app()->request->baseUrl; ?>/images/tit4.gif" /></a>
                    </div>
                    <div id="cont_w7check" class="w7Ttit2 <?php if(!$policy['set_cost'])echo 'hide'; ?>">
                        <div class="fee">
                             ...
                             <input type="radio" name="cost_mode" id="fee<?php echo $key; ?>" value="<?php echo $key; ?>" <?php if(intval($key)==$policy['cost_mode'])echo 'checked="checked"'; ?> onclick="changeCostNumDisplay(this)" /><label for="fee<?php echo $key; ?>"><?php echo $val; ?></label>
                             <?php } ?>
                        </div>
                        <ul>
                            <li class="w7Ttit"><strong class="mgl5">价格</strong></li>
                            <li class="help">
                                ￥<input name="price" id="price" type="text" value="<?php echo $policy['price']; ?>" onblur="setTotalCost()" class="txt1 txt7" />数量：<?php if($policy['cost_mode']>1&&$policy['cost_num']>0){ ?><span id="cost_num_box"><input type="text" name="cost_num" id="cost_num" class="select_box" onblur="setTotalCost()" value="<?php echo $policy['cost_num']; ?>" /></span><a href="javascript:void(0);" id="but_modify_cost_num" onclick="modifyCostNum()">不限</a><?php } else if($policy['cost_mode']>1) { ?><span id="cost_num_box">不限</span> <a href="javascript:void(0);" id="but_modify_cost_num"  onclick="modifyCostNum()">更改数量</a><?php } else { ?><span id="cost_num_box">不限</span> <a href="javascript:void(0);" id="but_modify_cost_num" class="hide" onclick="modifyCostNum()">更改数量</a><?php } ?>
                        ...
                        <ul>
                            ...
                            <li class="help">
                                ￥<span class="ml15" id="total_price">—</span>
                            </li>
                        </ul>
                        <br/>
                    </div>
                    <div class="mgl5">
                        <input id="w7check1" type="checkbox" <?php if($policy['set_limit_day']) echo 'checked="true"';?> /><label for="w7check1" class="w7Ttit1">限制每日投放数量</label>
                        <a class="toolTips_tag" onmouseover="showMyToolTips(this, '如何设置每天的投放数量？', '勾选后，您可以设置广告每天展现或点击的数量，到达该数量后，当天不再展现该广告。第二个投放日广告将再次展现，并从0开始重新计算当天的投放数量。')" onmouseout="hideMyToolTips()"  href="javascript:void(0);"><img src="<?php echo Yii::app()->request->baseUrl; ?>/images/tit4.gif" /></a>
                    </div>
                    <div id="cont_w7check1" class="w7Ttit2 <?php if(!$policy['set_limit_day'])echo 'hide'; ?>">
                        <strong>每天</strong>
                        <?php echo CHtml::dropDownList('limit_day_mode', @$policy['limit_day_mode'], $limitDayMode, array('class' => 'select_box ml15', 'id' => 'limit_day_mode')); ?>
                        <input type="text" id="limit_day_num" class="txt1 txt8 ml15" value="<?php echo $policy['limit_day_num']; ?>"/> <strong>次</strong>
                        <div id="error_cont_w7check1" class="hide errmsg">提示：数量请填写>0且≤1,000,000,000的整数。</div>
                    </div>
                    <div class="mgl5">
                        <input id="w7check2" type="checkbox" <?php if($policy['set_limit_one']) echo 'checked="true"';?> /><label for="w7check2" class="w7Ttit1">限制对独立访客的展现次数</label>
                        <a class="toolTips_tag" onmouseover="showMyToolTips(this, '如何限制对独立访客的展现次数？', '勾选后，您可以从每天、每小时、每分钟多个维度限制广告对独立访客的展现次数。')" onmouseout="hideMyToolTips()"  href="javascript:void(0);"><img src="<?php echo Yii::app()->request->baseUrl; ?>/images/tit4.gif" /></a>
                    </div>
                    <div id="cont_w7check2" <?php if(!$policy['set_limit_one']) echo 'class="hide"'; ?>>
                        ...
                        <div id="limit_one_<?php echo $key; ?>" class="w7Ttit2">
                            <?php echo CHtml::dropDownList('limit_one_mode_'.$key, @$val['mode'], $limitOneShowMode, array('class' => 'select_box', 'id' => 'limit_one_mode_'.$key, 'onchange'=>'checkLimitOneSet(this)')); ?>
                            最多展现 
                            <input type="text" name="limit_one_shownum_<?php echo $key; ?>" id="limit_one_shownum_<?php echo $key; ?>" class="txt1 txt8" value="<?php echo $val['num']; ?>"/> 次
                        ...
                        <?php } ?>
                        <?php } else {?>
                        <div id="limit_one_0" class="w7Ttit2">
                            <?php echo CHtml::dropDownList('limit_one_mode_0', @1, $limitOneShowMode, array('class' => 'select_box', 'id' => 'limit_one_mode_0', 'onchange'=>'checkLimitOneSet(this)')); ?>
                            最多展现 
                            <input type="text" name="limit_one_shownum_0" id="limit_one_shownum_0" class="txt1 txt8"/> 次
                            <span class="help"><a href="javascript:void(0);" onclick="removeCutDiv(this);">删除</a></span>
                            <div id="error_limit_one_0" class="hide errmsg">提示：已经设置此上限，请勿重复设置。</div>
                        </div>
                        <?php } ?>
                    </div>
                    <div id="add_limit_inputbox" class="w7Ttit2 <?php if(!$policy['set_limit_one']) echo 'hide'; ?>">
                        <span class="help"><a href="javascript:void(0);" onclick="addLimitOneInputBox();">添加多维</a></span>
                    </div>
                    </td>
                    </tr>
                    </table>
...
<?php if (isset($policy['ad_show']) && ($policy['ad_show']=='播放器' || $policy['ad_show']=='视频栏目' || $policy['ad_show']=='直播频道')): ?>
<div class="w6Table w7Table mainForm">
    <input type="hidden" name="ad_id" id="ad_id" value="<?php echo $policy['aid']; ?>" />
    <table border="0" cellpadding="0" cellspacing="0">
        <tbody>
            <tr>
                <td width="84" height="34" valign="top"><strong>播放器广告：</strong></td>
                <td class="ad_policy_frame">
                <div>
                    <strong class="mgl5">播放器状态</strong>
                    <?php foreach($playerCushion as $key=>$one):?>
                    <?php if ($policy['ad_show']!='直播频道' || $one!='片尾广告'):?>
                    <input type="radio" class="ml15" name="playerCushion" id="cushion_<?php echo $key; ?>" value="<?php echo $key; ?>" <?php if(intval($key)==$policy['cushion'])echo 'checked="checked"'; ?> onclick="setShowAttr()" />&nbsp;<label for="cushion_<?php echo $key; ?>"><?php echo $one; ?></label>
                    <?php endif;?>
                    <?php endforeach; ?>
                </div>
                <div class="feeAll mt15 cut-in-attr <?php if($policy['cushion']!=4) echo 'hide';?>">
                    <strong class="mgl5">展现方式</strong>
                    <?php foreach($showType as $key=>$one):?>
                    <input type="radio" class="ml15" name="show_type" id="show_type_<?php echo $key; ?>" value="<?php echo $key; ?>" <?php if(intval($key)==$policy['show_type'])echo 'checked="checked"'; ?> />&nbsp;<label for="show_type_<?php echo $key; ?>"><?php echo $one; ?></label>
                    <?php endforeach; ?>
                </div>
                <div class="feeAll mt15 cut-in-attr <?php if($policy['cushion']!=4) echo 'hide';?>">
                    <strong class="mgl5">展现时间</strong>
                    <input type="text" class="select_box ml15" name="show_time" id="show_time" value="<?php echo $policy['show_time']; ?>" /> (以秒为单位)
                </div>
                <div class="feeAll mt15">
                    <strong class="mgl5" style="margin-right:14px;">广告尺寸</strong>
                    宽度<input type="text" class="select_box ml15" name="width" id="width" value="<?php echo $policy['width']; ?>" />&nbsp;&nbsp;&nbsp;&nbsp;
                    高度<input type="text" class="select_box ml15" name="height" id="height" value="<?php echo $policy['height']; ?>"/>
                </div>
                <div class="feeAll mt15">
                    <strong class="mgl5" style="margin-right:14px;">广告位置</strong>
                    X坐标<input type="text" class="select_box ml15" name="pos_x" id="pos_x" value="<?php echo $policy['pos_x']; ?>"/>&nbsp;&nbsp;&nbsp;&nbsp;
                    Y坐标<input type="text" class="select_box ml15" name="pos_y" id="pos_y" value="<?php echo $policy['pos_y']; ?>"/>
                </div>
                </td>
            </tr>
        </tbody>
...
<div class="w6Table mainForm">
  <table border="0" cellpadding="0" cellspacing="0">
    <tbody>
      <tr>
        <td width="84" height="34"><strong>精准定向：</strong></td>
        <td><a href="javascript:void(0);" onClick="addDirectional();"><img src="<?php echo Yii::app()->request->baseUrl; ?>/images/btn10.gif" /></a>&nbsp;&nbsp;<a class="toolTips_tag" onmouseover="showMyToolTips(this, '什么是精准定向？', '精准定向是从各个维度对网站访客的筛选，实现根据访客的不同特征来展现不同的广告。')" onmouseout="hideMyToolTips()" href="javascript:void(0);"><img src="<?php echo Yii::app()->request->baseUrl; ?>/images/tit4.gif" /></a></td>
      </tr>
      <tr>
        <td width="84">&nbsp;</td>
        <td width="1000" id="directional_value"><ul class="directional_list" id="directional_list">
            <?php if(isset($directional['area_set'])) { ?>
            <li id="area_set" class="help"> <img onClick="delAreaSet();" src="<?php echo Yii::app()->request->baseUrl; ?>/images/deltit.gif"> 地域 = <?php echo $directional['area_set']['text'];?>
            ...
            <li id="connect_set" class="help"> <img onClick="delConnectSet();" src="<?php echo Yii::app()->request->baseUrl; ?>/images/deltit.gif"> 接入方式 = <?php echo $directional['connect_set']['text'];?>
            ...
            <li id="time_set" class="help"> <img onClick="delTimeSet();" src="<?php echo Yii::app()->request->baseUrl; ?>/images/deltit.gif"> 时间 = <?php echo $directional['time_set']['text'];?>
            ...
            <li id="btype_set" class="help"> <img onClick="delContentSet('btype');" src="<?php echo Yii::app()->request->baseUrl; ?>/images/deltit.gif"> 浏览器类型 = <?php echo $directional['btype_set']['text'];?>
            ...
            <li id="blanguage_set" class="help"> <img onClick="delContentSet('blanguage');" src="<?php echo Yii::app()->request->baseUrl; ?>/images/deltit.gif"> 浏览器语言 = <?php echo $directional['blanguage_set']['text'];?>
            ...
            <li id="osystem_set" class="help"> <img onClick="delContentSet('osystem');" src="<?php echo Yii::app()->request->baseUrl; ?>/images/deltit.gif"> 操作系统 = <?php echo $directional['osystem_set']['text'];?>
            ...
            <li id="resolution_set" class="help"> <img onClick="delContentSet('resolution');" src="<?php echo Yii::app()->request->baseUrl; ?>/images/deltit.gif"> 分辨率 = <?php echo $directional['resolution_set']['text'];?>
            ...
            <li id="from_url_set" class="help"> <img onClick="delUrlLimitSet('from_url');" src="<?php echo Yii::app()->request->baseUrl; ?>/images/deltit.gif"> 来源域 = <?php echo $directional['formurl_set']['value'];?>
            ...
            <li id="access_url_set" class="help"> <img onClick="delUrlLimitSet('access_url');" src="<?php echo Yii::app()->request->baseUrl; ?>/images/deltit.gif"> 被访url = <?php echo $directional['accessurl_set']['value'];?>
...
<div class="nextBtn"> <a href="javascript:void(0);" id="step_next"><img src="<?php echo Yii::app()->request->baseUrl; ?>/images/btn7.gif"></a><a href="javascript:void(0);" id="step_prev"><img src="<?php echo Yii::app()->request->baseUrl; ?>/images/btn8.gif"></a> </div>
...
<script type="text/javascript">
    $(function(){
        ...
        $("#step_prev").click(function(){
            var url = "<?php echo Yii::app()->createURL('videoAd/setAd');?>";
            frame_load(url);
            return false;
        })
        // 下一步点击事件
        $("#step_next").click(function(){
            submitForm();
            //var url = "<?php echo Yii::app()->createURL('videoAd/setLocate');?>";
            //frame_load(url);
            return false;
        })
        // 权重
    ...
    function addDirectional(){
        if ($("#directional").html()=="") {
            $("#directional").load("<?php echo Yii::app()->createURL('videoAd/directional');?>");
        } else {
    ...
    function saveAreaSet(){
        ...
        if ($("#area_set").length>0) {
            html = '<img src="<?php echo Yii::app()->request->baseUrl; ?>/images/deltit.gif" onclick="delAreaSet();"/>地域 '+mode;
            var pText = "";
            ...
            html = '<li id="area_set" class="help"><img src="<?php echo Yii::app()->request->baseUrl; ?>/images/deltit.gif" onclick="delAreaSet();"/>地域 '+mode;
            var pText = "";
            $("#address_select_box li").each(function(){
                var tmpid = $(this).attr("id");
                html += $("#"+tmpid+" span").html()+",";
    ...
    function saveConnectSet(){
        ...
        if ($("#connect_set").length>0) {
            html = '<img src="<?php echo Yii::app()->request->baseUrl; ?>/images/deltit.gif" onclick="delConnectSet();"/>接入方式 '+mode+' ';
            var pText = "";
            ...
            html = '<li id="connect_set" class="help"><img src="<?php echo Yii::app()->request->baseUrl; ?>/images/deltit.gif" onclick="delConnectSet();"/>接入方式 '+mode+' ';
            var pText = "";
            $("#connect_select_box li").each(function(){
                var tmpid = $(this).attr("id");
                html += $("#"+tmpid+" span").html()+",";
    ...
    function saveTimeSet(){
        ...
        if ($("#time_set").length>0) {
            var html = '<img src="<?php echo Yii::app()->request->baseUrl; ?>/images/deltit.gif" onclick="delTimeSet();"/>时间 '+mode+' ';
            var timeEnd = "";
            for(var i=1; i<8; i++) {
                html += "<span>"+week[i-1]+'：';
            ...
            var html = '<li id="time_set" class="help"><img src="<?php echo Yii::app()->request->baseUrl; ?>/images/deltit.gif" onclick="delTimeSet();"/>时间 '+mode+' ';
            var timeEnd = "";
            for(var i=1; i<8; i++) {
                html += "<span>"+week[i-1]+'：';
                if ($("#week_"+i).attr("checked")=="checked") {
    ...
    function saveSContentSet(index){
        ...
        if ($("#"+tag+"_set").length>0) {
            var html = '<img src="<?php echo Yii::app()->request->baseUrl; ?>/images/deltit.gif" onclick="delContentSet(\''+tag+'\');"/>'+arrTypeName[index]+' '+mode+' ';
            ...
            var html = '<li id="'+tag+'_set" class="help"><img src="<?php echo Yii::app()->request->baseUrl; ?>/images/deltit.gif" onclick="delContentSet(\''+tag+'\');"/>'+arrTypeName[index]+' '+mode+' ';
            $("#"+tag+"_select_box li").each(function(){
                var tmpid = $(this).attr("id");
                html += $("#"+tmpid+" span").html()+",";
                selectNum ++;
    ...
    function saveUrlLimitSet(index){
        ...
        var mode = ($("#directional_mode option:selected").val()==1)? "=" : "≠";
        if ($("#"+tag+"_set").length>0) {
            var html = '<img src="<?php echo Yii::app()->request->baseUrl; ?>/images/deltit.gif" onclick="delUrlLimitSet(\''+tag+'\');"/>'+arrTypeName[index]+' '+mode+' ';
            html += pText;
            ...
            var html = '<li id="'+tag+'_set" class="help"><img src="<?php echo Yii::app()->request->baseUrl; ?>/images/deltit.gif" onclick="delUrlLimitSet(\''+tag+'\');"/>'+arrTypeName[index]+' '+mode+' ';
            html += pText;
            html += '<textarea style="display:none;" name="'+tag+'_set_text" id="'+tag+'_set_text">'+pText+'</textarea>';
            html += '[<a href="javascript:void(0);" onclick="showDirectional('+index+')">修改</a>]</li>';
    ...
    function  selectProvinceAddr(provinceid) {
        if ($("#provice_"+provinceid).attr("checked")=="checked") {
            ...
            var addr = '<li id="address_select_'+provinceid+'"><span>'+addrName+'</span> <img class="mt7" src="<?php echo Yii::app()->request->baseUrl; ?>/images/view_clean.gif" onclick="deleteCurAddr(\''+provinceid+'\');"/></li>';
    ...
    function selectCityAddr(cityid){
        if ($("#city_"+cityid).attr("checked")=="checked") {
            ...
            var addr = '<li id="address_select_'+cityid+'"><span>'+addrName+'</span> <img class="mt7" src="<?php echo Yii::app()->request->baseUrl; ?>/images/view_clean.gif" onclick="deleteCurAddr(\''+cityid+'\');"/></li>';
            $("#address_select_box").append(addr);
            ...
            if (!selectNull && noSelectNum==1) {
                ...
                $("input[name='"+provinceid+"_city[]']").each(function(){
                    if ($(this).attr("checked")=="checked") {
                        ...
                        var addr = '<li id="address_select_'+cid+'"><span>'+addrName+'</span> <img class="mt7" src="<?php echo Yii::app()->request->baseUrl; ?>/images/view_clean.gif" onclick="deleteCurAddr(\''+cid+'\');"/></li>';
                        $("#address_select_box").append(addr);
                    }
                });
            }
    ...
    function addConnectMode(mode){
        ...
        var html = '<li id="conncet_select_'+mode+'"><span>'+name+'</span> <img class="mt7" src="<?php echo Yii::app()->request->baseUrl; ?>/images/view_clean.gif" onclick="deleteCurConncet(\''+mode+'\');"/></li>';
    ...
    function addSelectContent(mode, prefix){
        ...
        var html = '<li id="'+prefix+'_select_'+mode+'"><span>'+name+'</span> <img class="mt7" src="<?php echo Yii::app()->request->baseUrl; ?>/images/view_clean.gif" onclick="deleteCurContent(\''+mode+'\', \''+prefix+'\');"/></li>';
