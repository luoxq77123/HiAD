        ...
        <tbody>
            ...
            <tr>
                ...
                <div id="ggw_box" class="hide">
                  <?php $this->widget('MaterialListWidget', array('rote'=>'videoAd/getMaterialList', 'arrPageSize' => array(3 => 3, 10 => 10, 20 => 20), 'adTypeId' => 3, 'adShow' => 'player')); ?>
                </div>
                </td>
            </tr>
        </tbody>
...
$(document).ready(function(e) {
    ...
    $("#create_material").click(function(){
        var aid = $("#ad_id").val();
        var url = "<?php echo $this->createUrl('materialVideo/add');?>?backUrl=<?php echo $this->createUrl('videoAd/setMaterial');?>";
        frame_load(url);
    });
    $("#step_prev").click(function(){
        var url = "<?php echo Yii::app()->createURL('videoAd/setPolicy');?>";
        frame_load(url);
    });
    $("#step_next").click(function(){
        ...
        $.post(
            '<?php echo Yii::app()->createUrl("videoAd/setMaterial")?>',
            {'do':'save','aid':aid,'rotate':rotate,'rotateTime':rotateTime,'material':material}, 
            function(data){
                if(data.code < 1){
                    jAlert(data.msg);
                }else{
                    var url = "<?php echo Yii::app()->createURL('videoAd/list');?>";
                    frame_load(url);
                }
            },
            'json'
...
function user_search(){
    ...
    var url = '<?php echo Yii::app()->createUrl('videoAd/getMaterialList')?>?status='+search_status+'&type='+search_type+'&size='+search_size+'&name='+encodeURIComponent(search_name);
...
function completeMaterial(){
    ...
    $.post(
        '<?php echo Yii::app()->createUrl("videoAd/getMaterialInfo")?>',
        {'mids':mids}, 
        function(data){
            if(data.code < 1){
                jAlert(data.msg);
