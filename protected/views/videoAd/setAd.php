  ...
  <div id="ggw_box" <?php if (isset($ad['position_name'])) echo 'class="hide"';?>>
    ...
    <?php $this->widget('AdPositionListWidget', array(
        'arrPageSize' => array(3 => 3, 10 => 10, 20 => 20), 'adTypeId' => 3
    )); ?>
    <!--end 内容替换区-->
  </div>
...
$(function(){
    ...
    $("#add_new_ad").click(function(){
        ...
        var aid = $("#ad_id").val();
        $.post(
            '<?php echo Yii::app()->createUrl("videoAd/setAd")?>',
            {'do':'save','aid':aid,'pid':pid,'name':name,'order_id':order_id,'description':description}, 
            function(data){
                if(data.code < 1){
                    jAlert(data.message);
                }else{
                    var url = "<?php echo Yii::app()->createURL('videoAd/setPolicy?aid=');?>"+data.code;
                    frame_load(url);
                }
            },
            'json'
...
function user_search(){
    ...
    var url = '<?php echo Yii::app()->createUrl('videoAd/getAdPositionList')?>?status='+search_status+'&type='+search_type+'&name='+encodeURIComponent(search_name);
