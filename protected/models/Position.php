...
class Position extends CActiveRecord {
    ...
    public function rules() {
        return array(
            array('name,sort,ad_show_id', 'required', 'message' => '{attribute}不能为空', 'on' => 'add,edit'),
    ...
    public function getPositionList($search=array(), $initPageSize=3, $adTypeId=1){
        ...
        if ($adTypeId == 3) {
            $criteria->select = 'id,name,position_size,status,ad_show_id';
        } else {
        ...
        if ($adTypeId == 1) {
            ...
            $pager->route = 'videoAd/setAd';
            $spList = $this->findAll($criteria);
        }
