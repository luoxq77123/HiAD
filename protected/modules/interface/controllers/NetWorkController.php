<?php
class NetWorkController extends IBaseController {

    //对接函数
    public function actionindex(){ 
            Yii::import("application.helpers.Ccrul");
        $b = array('parameter'=>json_encode(array(
            'action' => 'getPermission',
            'appKey' => '123456789',
            'timestamp' => "2015-03-23 15:23:21",
            'token' => $_GET['token'],
            'type' => 1,
            'uid' => $_GET['uid'],
            )));
        $a = Ccurl::post('http://test.ida.sobeyyun.com/interface',$b);
        var_dump($_GET);exit;
    }
    /**
     * 用户注册接口
