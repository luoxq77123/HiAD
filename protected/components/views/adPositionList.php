        ...
        <tbody>
            ...
            <?php foreach ($spList as $key => $one): ?>
                <tr <?php echo ($key % 2 == 0) ? 'class="trBg"' : ''; ?>>
                    <td width="65"><input type="radio" name="ids" onclick="selectPosition(this);" class="checkbox_ids" value="<?php echo $one->id; ?>" /></td>
                    <td id="pname_<?php echo $one->id; ?>"><?php echo $one->name; ?></td>
                    <td><?php echo $one->status == 1 ? '启用' : '禁用'; ?></td>
                    <td id="ptype_<?php echo $one->id; ?>"><?php echo (isset($adShows[$one->ad_show_id]))?$adShows[$one->ad_show_id]: '未定义'; ?></td>
                    <td id="site_<?php echo $one->id; ?>"><?php if($adTypeId==1){echo isset($sites[$one->site_id]) ? $sites[$one->site_id] : '-';}else if($adTypeId==2){echo isset($sites[$one->app_id]) ? $sites[$one->app_id] : '-';} ?></td>
                    <td id="psize_<?php echo $one->id; ?>"><?php echo $one->position_size; ?></td>
                    <td>&nbsp;</td>
                </tr>
            <?php endforeach; ?>
            <?php else:?>
