...
<div class="san_nav">
    <ul class="fl san_list" id="san_list">
        ...
        <!--<li><a href="<?php echo Yii::app()->createUrl('thridPosition/getCode'); ?>" class="load_frame">获取代码</a></li>-->
    </ul>
</div>
<!--end 导航-->
...
<!--生成代码-->
<div class="bgline pl_30 add"><div style="float:left; margin-top:10px;">广告位：</div><a href="<?php echo Yii::app()->createUrl('thridPosition/add');?>" id="add_position" title="新建广告位" class="iscbut cbut_jia load_frame"><span>新建广告位</span></a></div>
...
<div class="tpboder adbox">
    <table border="0" cellspacing="0" cellpadding="0" class="list_table" id="list_table">
        ...
        <td>
            <a href="<?php echo Yii::app()->createUrl('thridPosition/edit', array('id' => $one->id));?>" class="load_frame" title="修改广告位 <?php echo $one->name;?>">修改</a> |
            <a href="<?php echo Yii::app()->createUrl('statistics/thrid', array('type' => 'position', 'ad_id' => $one->id, 'ad_name' => urlencode($one->name)));?>" class="load_frame">统计</a>
        </td>
        </tr>
        <?php endforeach;?>
          <?php else:?>
